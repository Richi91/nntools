...
MAX_OUTPUT_SEQ_LEN = len(trainDataset.dimensions['maxLabelLength']) #778
MAX_INPUT_SEQ_LEN = len(trainDataset.dimensions['maxLabelLength']) #778
#*******************************************************************************************************
print("TODO!!!! OUTPUTSEQLEN AUCH IM FRONTEND")
...
net_outputs = np.array([forward_pass(x,mask)[0] for x, mask in zip(x_val_batch,x_val_mask)]) 
...
decoded = [be.decodeSequenceNoCTC(sequence_probdist[i], x_val_masks[i]) for i in range(sequence_probdist.shape[0])]

# calculate PER for each training datum sequentially. 
# TODO: PER in batches
PERs = [be.calcPERNoCTC(tar[i,x_val_masks[i,:]==1], decoded[i]) for i in range(tar.shape[0])]
...
for epoch in range(N_EPOCHS):  
    ...
    cost_val = np.array([compute_cost(x, y, mask)[()] \
        for x, y, mask
        in zip(x_val_batch, y_val_batch, x_val_mask)]).mean()  
